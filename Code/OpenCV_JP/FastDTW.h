...
public:
	...
	fastdtw(uint n, uint constraint) : mGamma(n, vector<double>(n, INF)), mN(n), mConstraint(constraint) {
	}
	/*
...
public:
	LB_Improved(const vector<double> &  v, int constraint) : NearestNeighbor(v, constraint), lb_keogh(0), full_dtw(0), V(v), buffer(v), mConstraint(constraint),
		bestsofar(fastdtw::INF), U(V.size(), 0), L(V.size(), 0), U2(V.size(), 0), L2(V.size(), 0) {
