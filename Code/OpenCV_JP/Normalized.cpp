...
void Normalized::MeasurementOfAverageStrokeThickness()
{
	...
	this->AST /= new_order == 0 ? 1 : new_order;
	delete[] val;
}
...
void Normalized::StraightLineRemoval()
{
	...
	for (int j = 0; j < this->word.rows * 3 / 4; j++)
	{
		for (int i = 0; i < this->word.cols; i++)
		{
			Scalar intensity = this->word.at<uchar>(j, i);
						...
						for (int l = j; l < this->word.rows; l++) {
							...
							if (intensity[0] <= THRESHOLD_INK)
								...
								if (sum_rows != 0 & sum_rows <= AST)
									cv::line(this->word, cv::Point(k, j - 2), cv::Point(k, l), Scalar(255, 255, 255));
								sum_rows = 0;
								break;
							}
						}
...
void Normalized::UnderlineRemoval()
{
	int sum = 0;
	int sum_rows = 0;
	cout << "peak_line =  " << peak_line << endl;
	if (peak_line >= this->word.rows * 3 / 4) {
		cout << "A skewed underline may be present.";

		for (int j = this->word.rows * 3 / 4; j < this->word.rows; j++)
		{
			for (int i = 0; i < this->word.cols; i++)
			{
				Scalar intensity = this->word.at<uchar>(j, i);
...
Mat Normalized::SkewDetectionUsesCentreOfMass(int upper, int lower)
{
	...
	if (theta <= 25) {
		...
		Mat mat1 = Mat(cv::Size(word.size().width + 100, word.size().height + 100), CV_8UC1, Scalar(255, 255, 255));;
		Mat rs = mat1.clone();
		word.copyTo(mat1(cv::Rect(word.size().width / 2, 50, word.cols, word.rows)));

		cv::Point2f ptCp(0, 0);
		cv::Mat M = cv::getRotationMatrix2D(ptCp, theta, 1);
		cv::warpAffine(mat1, mat1, M, mat1.size(), INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255)); //Nearest is too rough, 

		MatModifies matModifies;
		int x_min_word, y_min_word, x_max_word, y_max_word;
		matModifies.findMinMax(mat1, x_min_word, y_min_word, x_max_word, y_max_word);
		word = matModifies.cropImage(mat1, x_min_word, y_min_word, x_max_word, y_max_word);
	}
	else {
...
Mat Normalized::preprocess1(Mat& im)
{
...
Mat Normalized::rot(Mat& im, double thetaRad)
{
	cv::Mat rotated;
