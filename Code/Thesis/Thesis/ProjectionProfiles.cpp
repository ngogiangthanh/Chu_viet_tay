#include "ProjectionProfiles.h"

ProjectionProfiles::ProjectionProfiles(Mat src)
{
	this->source = src.clone();
}

void ProjectionProfiles::cal_pp()
{
...
float ProjectionProfiles::I(int r, int c)
{
	Scalar intensity = this->source.at<uchar>(r, c);
	return intensity[0];
}

int ProjectionProfiles::get_pp(float *&returnVal)
{
...
Mat ProjectionProfiles::draw_pp()
{
	...
	//Show
	//imshow("ProjectionProfiles profile", dist);
